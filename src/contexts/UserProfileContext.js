...
export default function UserProfileContextProvider({children}){
    const [profile,profileDispacher] = useReducer(UserProfileReducer,InitialValueUserProfile);
    ...
    const getUserProfile =async(userId,username)=>{
        ...
        try{
            ...
            userFeedDispacher({type:"USER_PROFILE",payload:{data:filtered, value:"userProfileView"}})
            profileDispacher({type:"USER_PROFILE",payload:{userData:responseData.user}})
    ...
    return(<UserProfileContext.Provider value={{profile,profileDispacher,editUserProfile,getUserProfile}}>{children}</UserProfileContext.Provider>)
}
